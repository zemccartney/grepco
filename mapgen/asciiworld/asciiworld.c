...
void
screen_draw_segment(struct screen *s, double x1, double y1, double x2, double y2)
{
    if (fabs(x1 - x2) < 0.1 * s->width &&
        fabs(y1 - y2) < 0.1 * s->height)
    {
        gdImageLine(s->img, x1, y1, x2, y2, s->brush);
    }
}
                             ...
                             double lat_deg, double r_deg)
{
    ...
    for (i = 0; i < steps + 1; i++)
    {
        ...
        /* Project and maybe draw line segment. */
        (s->project)(s, lon_deg, lat_deg, &x2, &y2);
        if (i >= 1)
    ...
    while (getline(&line, &line_n, fp) != -1)
    {
        if (strncmp(line, "points", strlen("points")) == 0)
        {
                   ...
                   strncmp(line, ".\n", strlen(".\n")) != 0)
            {
                if (sscanf(line, "%lf %lf\n", &lat, &lon) == 2)
                {
                    (s->project)(s, lon, lat, &x1, &y1);
                    gdImageSetPixel(s->img, x1, y1, s->brush);
                }
            }
        }
    }
